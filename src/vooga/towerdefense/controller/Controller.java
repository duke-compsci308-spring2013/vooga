...
import vooga.towerdefense.factories.examples.MergeConflictTower;
...
public class Controller {
    ...
    public Controller (String language) {
        ...
        ExampleAuraTowerFactory codeStyleGenerator = new ExampleAuraTowerFactory(map, "Tree of Doom", null);
        codeStyleGenerator.initialize(map);
        GameElement duvallTheMighty = codeStyleGenerator.createElement(new Location(450, 200));
        map.addGameElement(duvallTheMighty);
        //MergeConflictTower buildMorePylons = new MergeConflictTower(map, "RAGE", null);
        //buildMorePylons.initialize(map);
        //GameElement github = buildMorePylons.createElement(new Location(250, 400));
        //map.addGameElement(github);
        waves.add(WaveFactory.createWave(new ExampleUnitFactory("Troll", new TrollUnitDefinition(), map), 25, map, map.getTile(new Point(25, 275))));
    ...
    public void displayElementInformation (GameElement e) {
        if (e != null) {
            ...
            if (e.getAttributeManager().hasUpgrades()) {
                List<String> upgrades = new ArrayList<String>(e.getAttributeManager().getUpgrades().keySet());
                myView.getGameElementInfoScreen().displayUpgradesAndButton(upgrades);
            }
        }
        else {
    ...
    private GameElement createNewElement (GameElement item) {
        try {
            ...
            Class[] types =
            { Pixmap.class, Location.class, Dimension.class, AttributeManager.class,
             List.class };
            Constructor<? extends GameElement> constructor = myClass.getConstructor(types);
            Object[] parameters =
            { item.getPixmap(), item.getCenter(), item.getSize(),
             item.getAttributeManager(), item.getActions() };
    ...
    public void handleShopClickOnItem (Point p) {
        ...
        GameElement t = factory.createTower(new Location(p.getX(), p.getY()));
        // Tower t = new Tower(new Pixmap("shopicons/redtower.png"), new Location(p.getX(),
        // p.getY()), new Dimension(50, 50), null, actions);
    ...
    public void upgradeSelectedItemTo (String upgradeName) {
        GameElement t = (SelectMode) myControlMode)
