...
//    private CollisionManager myCollisionManager;
    ...
    /**
     * Constructor registering the model (to specify and delegate collisions handling)
     * @param m
     * @param level
     */
    public SpriteManager (Model m, Level level) {
        myLevel = level;
        myModel = m;
        mySprites = new ArrayList<Sprite>();
        initFrames();
        ...
        for (int i = 0; i < mySprites.size(); i++) {
            for (int j = 0; j < mySprites.size(); j++) {
                ...
                if (obj1.intersects(obj2)) {
                    getCollisionManager().handleCollision(obj1, obj2);
                }
            }
        }
    ...
    private CollisionManager getCollisionManager () {
        return myModel.getCollisionManager();
    }
